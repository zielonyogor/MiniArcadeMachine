    ...
    def update(self):
        if self.state == 'lose':
            if input.get_button_input() == input.SELECT:
                ...
                if db.is_better_simon_says(self.score):
                    self.game_state_manager.enter_name_scene('simon_says', self.score)
                else:
        ...
        key = input.get_joystick_input()
        if key == input.LEFT:
            if self.simon == 0:
                music_thread = threading.Thread(target=buzzer.play_left_melody, daemon=True)
                music_thread.start()
                self.win()
            else:
                music_thread = threading.Thread(target=buzzer.play_sad_melody, daemon=True)
                music_thread.start()
                self.wrong = 0
                self.state = 'lose'
        elif key == input.RIGHT:
            if self.simon == 1:
                music_thread = threading.Thread(target=buzzer.play_right_melody, daemon=True)
                music_thread.start()
                self.win()
            else:
                music_thread = threading.Thread(target=buzzer.play_sad_melody, daemon=True)
            ...
            if self.simon == 2:
                music_thread = threading.Thread(target=buzzer.play_up_melody, daemon=True)
                music_thread.start()
                self.win()
            else:
                music_thread = threading.Thread(target=buzzer.play_sad_melody, daemon=True)
            ...
            if self.simon == 3:
                music_thread = threading.Thread(target=buzzer.play_down_melody, daemon=True)
                music_thread.start()
                self.win()
            else:
                music_thread = threading.Thread(target=buzzer.play_sad_melody, daemon=True)
